
\subsection*{Kennung des Teilnehmers: \rule{3cm}{.5pt}
\hspace{1cm}Datum: \rule{2cm}{.5pt}}

\subsection*{Vorbetrachtungen}

\begin{enumerate}
\item Wie gro{\ss} sind Ihre Erfahrungen in C ?  
[bitte in Skala "`Sch"atzung'' ankreuzen]

{\small
\begin{tabular}{|l||*{6}{p{1.6cm}|}}
\hline
Sch\"atzung & \multicolumn{6}{c|}{} \\ \hline
Wert & \centering 0  &  \centering 1 & \centering 2 & \centering 3 & \centering 4 &\ \ \ \  5 \\
\hline
\hline
Vergleichs-  & gar   & Grund-  & Prak-  & Neben-       & beruf- & Guru \\
basis:       & nicht & studium & tikum  & t\"atigkeit  & lich   & \\
\hline
\end{tabular}
}

\item Wieviele Jahre haben Sie mit C gearbeitet? \rule{1cm}{.5pt}

%\item Kannten Sie strukturelles Testen 
%au{\ss}er durch Vorlesung/"Ubung schon vorher? Woher?\\
%\vspace{1cm}


\item Wie motiviert sind Sie f\"ur das Experiment ? [ankreuzen]

{\small
\begin{tabular}{|l||*{6}{p{1.6cm}|}}
\hline
Sch\"atzung & \multicolumn{6}{c|}{} \\ \hline
Wert & \centering 0 & \centering 1 & \centering 2 & \centering 3 & \centering 4 &\ \ \ \  5 \\
\hline
\hline
Vergleichs-  & gar   & wenig & k"onnte     & mittel-  & gut   & sehr \\
basis:       & nicht &       & besser sein & m"a"sig  &       & gut \\
\hline
\end{tabular}
}

\end{enumerate}

\subsection*{Ergebnisse}

Bitte ziehen Sie bei den zeitbezogenen Angaben Ihre Zeit f\"ur Pausen,
Essengehen usw.~ab. 

\begin{enumerate}
\addtocounter{enumi}{3}
\item Wann haben Sie mit dem Experiment angefangen ? 
\rule{7mm}{.5pt} : \rule{7mm}{.5pt} [Stunde:Minute]

\item Wieviel Zeit haben Sie am Anfang zur Generierung von Tests
verwendet ?\\ 
\rule{7mm}{.5pt} : \rule{7mm}{.5pt} [Stunden:Minuten]

\item Bitte Tragen Sie hier ein, bei welchem Rechner und zu welchem
Zeitpunkt Sie mit Ihren Tests angefangen haben:
\rule{4cm}{.5pt} ,
\rule{7mm}{.5pt} : \rule{7mm}{.5pt} [Stunde:Minute]

\item Wieviele Testf\"alle haben Sie letztendlich generiert ? 
\rule{7mm}{.5pt} [Anzahl Testf\"alle]

\item Geben Sie die Coverage-Werte an, die Sie mit
Ihren Testf\"allen erreicht haben ("`fully satisfied"'). Binary 
branch \rule{7mm}{.5pt} [\%],
Switch \rule{7mm}{.5pt} [\%], 
Loop \rule{7mm}{.5pt} [\%], 
Operator Instrumentation \rule{7mm}{.5pt} [\%], 
Summary of all Condition Types \rule{7mm}{.5pt} [\%]

\item Wie lange haben Sie f\"ur das Eintippen, Durchf\"uhren, und
weitere Generierung von Testf\"allen gebraucht?
\rule{7mm}{.5pt} : \rule{7mm}{.5pt} [Stunden:Minuten]

\item Wann haben Sie mit der Suche nach Fehlverhalten angefangen ? \\
\rule{7mm}{.5pt} : \rule{7mm}{.5pt} [Stunde:Minute]

\item Wie lange haben Sie aktiv an der Suche nach Fehlverhalten gearbeitet ? \\
\rule{7mm}{.5pt} : \rule{7mm}{.5pt} [Stunden:Minuten]

\item Wann haben Sie mit der Fehlerisolation angefangen ?
\rule{7mm}{.5pt} : \rule{7mm}{.5pt} [Stunde:Minute]

\item Wie lange haben Sie aktiv an der Fehlerisolation gearbeitet ? 
\rule{7mm}{.5pt} : \rule{7mm}{.5pt} [Stunden:Minuten]

\item Wann haben Sie das Experiment beendet ? 
\rule{7mm}{.5pt} : \rule{7mm}{.5pt} [Stunde:Minute]

\item Haben Sie Ihrer Meinung nach alle Fehler gefunden ? Wenn nein, geben
Sie bitte eine Sch\"atzung an wieviele Sie entdeckt haben:
\rule{7mm}{.5pt} \% [Prozent]

\item Wie gut beherrschen Sie strukturelles Testen (Ihrer Meinung nach) ? [ankreuzen]

{\small
\begin{tabular}{|l||*{6}{p{1.6cm}|}}
\hline
Sch\"atzung & \multicolumn{6}{c|}{} \\ \hline
Wert & \centering 0 & \centering 1 & \centering 2 & \centering 3 & \centering 4
&\ \ \ \  5 \\
\hline
\hline
Vergleichs-  & gar   & wenig & k"onnte     & mittel-  & gut   & sehr \\
basis:       & nicht &       & besser sein & m"a"sig  &       & gut \\
\hline
\end{tabular}
}

\end{enumerate}

