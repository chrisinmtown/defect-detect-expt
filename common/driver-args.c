/* driver-args.c: capture and mail argument vector, then invoke mymain */
     ...
     char **argv;
{
  ...
  char msg[10 * BUFSIZ], argbuf[9 * BUFSIZ];
  ...
  if (argc == 1) {
    ...
    capture_argv(argc, argv, argbuf);
    sprintf(msg, "%s %s %s", METHOD, *argv, argbuf);
    send_mail(MAILER, TARGET, msg, (char *)0);
  }
